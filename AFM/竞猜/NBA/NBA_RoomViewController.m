...
@interface NBA_RoomViewController ()<UITableViewDelegate , UITableViewDataSource , BtnBackBlcok>{
    NSInteger numPerPage;//页数
    
}
@property (nonatomic , strong) TopView *topView;                     //顶部视图
@property (nonatomic , strong) ClooseRoomView *clooseRView;          // 
@property (nonatomic , strong) NSMutableArray *listData;             //
@property (nonatomic , strong) NSMutableArray *joinData;             //参加次数

@end

@implementation NBA_RoomViewController

NSString *Games_cellad = @"GameTableViewCell";
...
//初始化导航栏
- (void)initNavigation {
    self.navigationItem.title = @"NBA房";
}
...
- (void)initView {
    ...
    [self.tableView registerClass:[GameTableViewCell class] forCellReuseIdentifier:Games_cellad];
    [self.view addSubview:self.tableView];
    self.Btn_tag = @"1";
    //downview
    self.men_Lab.text = [NSString stringWithFormat:@"%@", self.appCache.menPiao];
...
- (void)initListData {
    if ([self.Btn_tag isEqualToString:@"1"]) {
        self.projectId = @"4";
        self.typeId = @"0";
    }else if ([self.Btn_tag isEqualToString:@"2"]){   //五人房
        self.projectId = @"4";
        self.typeId = @"2";

    }else if ([self.Btn_tag isEqualToString:@"3"]){   //八人房
        self.projectId = @"4";
        self.typeId = @"4";
    ...
    [manager POST: urlStr parameters:paramters progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if ([[dic objectForKey:@"error"]isEqualToNumber:@0]) {
            NBAListModel *listModel = [NBAListModel yy_modelWithDictionary:dic];
            if (numPerPage == 1) {
            ...
            for (NBAListModelCell *model in self.listData) {
                self.joinData = [NSMutableArray arrayWithObject:model.join_num];
            }
            self.num_Lab.text = [NSString stringWithFormat:@"%@ 场",[self.joinData valueForKeyPath:@"@sum.floatValue"]];        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    GameTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:Games_cellad];
...
//筛选按钮
- (IBAction)clooseBtnClick:(UIButton *)sender {
...
@implementation NBAListModel
+ (NSDictionary *)modelContainerPropertyGenericClass {
    return @{@"data" : [NBAListModelCell class]};
}
@end

@implementation NBAListModelCell
