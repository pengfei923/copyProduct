...
@interface DJ_RoomViewController ()<UITableViewDelegate , UITableViewDataSource , BtnBackBlcok>{
...
@implementation DJ_RoomViewController

NSString *DJs_cellad = @"GameTableViewCell";
...
- (void)initView {
    ...
    [self.topView.button4 setTitle:@"活动房" forState:UIControlStateNormal];
    ...
    [self.topView.button4 setTitle:@"活动房" forState:UIControlStateSelected];
    ...
    [self.tableView registerClass:[GameTableViewCell class] forCellReuseIdentifier:DJs_cellad];
...
- (void)initListData{
    ...
    [manager POST: urlStr parameters:paramters progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if ([[dic objectForKey:@"error"]isEqualToNumber:@0]) {
            DJListModel *listModel = [DJListModel yy_modelWithDictionary:dic];
            if (numPerPage == 1) {
            ...
            for (DJListModelCell *model in self.listData) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    GameTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:DJs_cellad];
    DJListModelCell *model = [self.listData objectAtIndex:indexPath.row];
...
@implementation DJListModel
+ (NSDictionary *)modelContainerPropertyGenericClass {
    return @{@"data" : [DJListModelCell class]};
}
@end

@implementation DJListModelCell
