...
@interface StoreTableHeaderView ()<ZFPageScrollViewDataSource , ZFPageScrollViewDelegate>{
...
- (UIView*)pageScrollView:(ZFADScrollView *)pageScrollView viewForRowAtIndex:(int)index {
    CarouselModelCell *model = [m_ad_array objectAtIndex:index];
    UIImageView *view = [[UIImageView alloc]init];
...
- (void)startUpSystemCarousel {
    ...
    [manager POST: urlStr parameters:paramters progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if ([[dic objectForKey:@"error"]isEqualToNumber:@0]) {
            CarouselModel *tempmodel = [CarouselModel yy_modelWithDictionary:dic];
...
+ (NSDictionary *)modelContainerPropertyGenericClass {
    return @{@"data" : [CarouselModelCell class]};
}
@end

@implementation CarouselModelCell
